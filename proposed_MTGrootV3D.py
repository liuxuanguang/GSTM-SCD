...
import random
class MTGrootV3D_SV3_small(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV3_small, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
        ...
        self.MambaLayer = STM_GrootV3D_V2_small(768, False)
        self.seg_conv = nn.Sequential(
            nn.Conv2d(256, 128, 3, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(True),
    ...
    def forward(self, x1, x2, x3):
        ...
        for i in range(len(feature1)):
            ...
            feature_diff.append(self.CFEM[i](feature[pair[0]][i], feature[pair[0]][i]))
        xc = self.CD_Decoder(feature_diff)
        change = self.classifierCD(xc)
        change = F.interpolate(change, size=(h, w), mode='bilinear', align_corners=False)
        seg1 = F.interpolate(self.seg_conv(out_1), size=(h, w), mode='bilinear', align_corners=False)
        ...
        return [seg1, seg2, seg3], change.squeeze(1), pair
class MTGrootV3D_SV3_WUSU_Random(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV3_WUSU_Random, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
        ...
        self.classifierCD = nn.Sequential(nn.Conv2d(256, 64, kernel_size=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout(),
                                          nn.Conv2d(64, 1, kernel_size=1))
        updated_weights = {}
        # pretrained_weights = torch.load('/media/lenovo/课题研究/博士小论文数据/长时序变化检测/Long-term-SCD/CMSCD_lxg/checkpoints/WUSU/WUSU_proposed-TLLOSS-0312/resnet34/epoch88_Score54.19_mIOU82.58_Sek42.03_Fscd76.45_OA95.82.pth')
        # # 1. 过滤出预训练权重中在模型字典中也存在的键
        # for key, value in pretrained_weights.items():
        #     if key.startswith('backbone.'):
        #         new_key = key.replace('backbone.', '')
...
class MTGrootV3D_SV3_WUSU(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV3_WUSU, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
...
class MTGrootV3D_SV3_DynamicEarth(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV3_DynamicEarth, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
...
# LXG:baseline
class MTGrootV3D_SV3_DynamicEarth_random_AE2(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV3_DynamicEarth_random_AE2, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
...
class MTGrootV3D_SV3_DynamicEarth_random_base(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV3_DynamicEarth_random_base, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
        ...
        self.classifierCD = nn.Sequential(nn.Conv2d(256, 64, kernel_size=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout(),
                                          nn.Conv2d(64, 1, kernel_size=1))
        updated_weights = {}
        # pretrained_weights = torch.load('/media/lenovo/课题研究/博士小论文数据/长时序变化检测/Long-term-SCD/CMSCD_lxg/checkpoints/DynamicEarth/MTGrootV3D_SV3_DynamicEarth_random_base_0417/resnet34/epoch75_Score56.00_mIOU89.08_Sek41.81_Fscd65.34_OA78.12.pth')
        # # 1. 过滤出预训练权重中在模型字典中也存在的键
        # for key, value in pretrained_weights.items():
        #     if key.startswith('backbone.'):
        #         new_key = key.replace('backbone.', '')
    ...
    def forward(self, x1, x2, x3, x4, x5, x6):
        ...
        pairs = sorted({tuple(sorted(p)) for p in pairs})  # 去重并排序
        ...
        for i in range(len(feature1)):
            # feature_diff.append(self.CFEM[i](feature[pair[0]][i], feature[pair[-1]][i]))
            # feature_diff.append(self.CFEM[i](feature1[i], feature6[i]))
            feature_diff.append(feature[pair[0]][i]-feature[pair[-1]][i])
        xc = self.CD_Decoder(feature_diff)
...
# LXG:共用解码器
class MTGrootV3D_SV4_DynamicEarth(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(MTGrootV3D_SV4_DynamicEarth, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
    ...
    def forward(self, x1, x2, x3, x4, x5, x6):
        ...
        out_3 = self.Seg_Decoder1(feature3)
        out_4 = self.Seg_Decoder1(feature4)
        out_5 = self.Seg_Decoder1(feature5)
        out_6 = self.Seg_Decoder1(feature6)
        # CDDecoder
        feature_diff = []
        for i in range(len(feature1)):
            feature_diff.append(self.CFEM[i](feature5[i], feature6[i]))
        xc = self.CD_Decoder(feature_diff)
        change = self.classifierCD(xc)
        change = F.interpolate(change, size=(h, w), mode='bilinear', align_corners=False)
        seg1 = F.interpolate(self.seg_conv(out_1), size=(h, w), mode='bilinear', align_corners=False)
...
class ST_VSSM_Siam(nn.Module):
    def __init__(self):
        super(ST_VSSM_Siam, self).__init__()

        self.backbone = GrootV(depths=[2, 2, 9, 2])

    def forward(self, x1, x2, x3):

        feature_xy1 = self.backbone.forward(x1)
        feature_xy2 = self.backbone.forward(x2)
        feature_xy3 = self.backbone.forward(x3)
        # 初始化feature1和feature2列表
        return feature_xy1, feature_xy2, feature_xy3



# 计算encoder的params和FLOPs
class ST_VSSM(nn.Module):
    def __init__(self):
        super(ST_VSSM, self).__init__()
...
if __name__ == '__main__':
    ...
    model = MTGrootV3D_SV3_WUSU(backbone='GrootV', pretrained=False, nclass=13, lightweight=True, M=6, Lambda=0.00005).to(device)
    print(model)
