...
# from models.proposed_MTGrootV3D import MTGrootV_SV3_WUSU as Net
...
class TemporalConsistencyLoss(nn.Module):
    ...
    def forward(self, change_ab, change_bc, change_ac):
        """
        :param change_ab: 时相a到时相b的变化概率 [B, H, W] (0~1)
        :param change_bc: 时相b到时相c的变化概率 [B, H, W]
        :param change_ac: 时相a到时相c的变化概率 [B, H, W]
        :return: 逻辑一致性损失值
        """
        # 逻辑规则1：a变且b未变 -> a-c必须变
        mask_rule1 = (change_ab > 0.5) & (change_bc < 0.5)
        loss_rule1 = torch.masked_select((change_ac - 1)**2, mask_rule1)

        # 逻辑规则2：a未变且b变 -> a-c必须变
        mask_rule2 = (change_ab < 0.5) & (change_bc > 0.5)
        loss_rule2 = torch.masked_select((change_ac - 1)**2, mask_rule2)

        # 逻辑规则3：a未变且b未变 -> a-c必须未变
        mask_rule3 = (change_ab < 0.5) & (change_bc < 0.5)
        loss_rule3 = torch.masked_select((change_ac - 0)**2, mask_rule3)

        # 合并所有损失项
        total_loss = 0.0
        if len(loss_rule1) > 0:
            total_loss += loss_rule1.mean()
        if len(loss_rule2) > 0:
            total_loss += loss_rule2.mean()
        if len(loss_rule3) > 0:
            total_loss += loss_rule3.mean()
...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--Net_name", type=str, default="BiSRNet_MT_noignore_0623")
...
class Trainer:
    ...
    def training(self, epoch):
        ...
        for i, (img1, img2, img3, mask1, mask2, mask3, mask_bn, id) in enumerate(tbar):
            ...
            #  加逻辑一致性损失用这个
            out1, out2, out3, out_bn12, out_bn23, out_bn13 = self.model(img1, img2, img3)    #[b,6,512,512],[b,6,512,512],[b,512,512]
            loss1 = self.criterion_seg(out1, mask1)
            loss2 = self.criterion_seg(out2, mask2)
            loss3 = self.criterion_seg(out3, mask3)
            loss_seg = (loss1 + loss2 + loss3) / 3
            ...
            loss_bn_2 = self.criterion_bn_2(out_bn.float(), mask_bn)
            loss_bn = loss_bn_1 + loss_bn_2
            loss_TL = self.TCL(out_bn12, out_bn23, out_bn13)
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            for img1, img2, img3, mask1, mask2, mask3, mask_bn, id in tbar:
                ...
                out1, out2, out3, out_bn = self.model(img1, img2, img3)
