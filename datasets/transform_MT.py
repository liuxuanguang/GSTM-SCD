...
# def rand_flip_MCD(img1, img2, label1, label2, label_contour):
#     r = random.random()
#     # showIMG(img.transpose((1, 2, 0)))
#     if r < 0.25:
#         return img1, img2, label1, label2, label_contour   # right
#     elif r < 0.5:
#         return np.flip(img1, axis=0).copy(), np.flip(img2, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(label2, axis=0).copy(), np.flip(label_contour, axis=0).copy()
#     elif r < 0.75:
#         return np.flip(img1, axis=1).copy(), np.flip(img2, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(label2, axis=1).copy(), np.flip(label_contour, axis=1).copy()
#     else:
#         return img1[::-1, ::-1, :].copy(), img2[::-1, ::-1, :].copy(), label1[::-1, ::-1].copy(), label2[::-1, ::-1].copy(), label_contour[::-1, ::-1].copy()
...
def rand_flip_MCD(img_A, img_B, img_C, label_A, label_B, label_C, label12, label23):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img_A, img_B, img_C, label_A, label_B, label_C, label12, label23
    elif r < 0.5:
        return np.flip(img_A, axis=0).copy(), np.flip(img_B, axis=0).copy(), np.flip(img_C, axis=0).copy(), np.flip(label_A, axis=0).copy(), np.flip(label_B, axis=0).copy(), np.flip(label_C, axis=0).copy(), np.flip(label12, axis=0).copy(), np.flip(label23, axis=0).copy()
    elif r < 0.75:
        return np.flip(img_A, axis=1).copy(), np.flip(img_B, axis=1).copy(), np.flip(img_C, axis=1).copy(), np.flip(label_A, axis=1).copy(), np.flip(label_B, axis=1).copy(), np.flip(label_C, axis=1).copy(), np.flip(label12, axis=1).copy(), np.flip(label23, axis=1).copy()
    else:
        return img_A[::-1, ::-1, :].copy(), img_B[::-1, ::-1, :].copy(), img_C[::-1, ::-1, :].copy(), label_A[::-1, ::-1].copy(), label_B[::-1, ::-1].copy(), label_C[::-1, ::-1].copy(), label12[::-1, ::-1].copy(), label23[::-1, ::-1].copy(),
...
def rand_rot90_MCD(img_A, img_B, img_C, label_A, label_B, label_C, label12, label23):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img_A, img_B, img_C, label_A, label_B, label_C, label12, label23
    else:
        return np.rot90(img_A).copy(), np.rot90(img_B).copy(), np.rot90(img_C).copy(), np.rot90(label_A).copy(), np.rot90(label_B).copy(), np.rot90(label_C).copy(), np.rot90(label12).copy(), np.rot90(label23).copy()
...
# def rand_rot90_flip_MCD(img1, img2, label1, label2, label_contour):
#     img1, img2, label1, label2, label_contour = rand_rot90_MCD(img1, img2, label1, label2, label_contour)
#     return rand_flip_MCD(img1, img2, label1, label2, label_contour)
...
#LT processing
def rand_rot90_flip_MCD(img_A, img_B, img_C, label_A, label_B, label_C, label12, label23):
    img_A, img_B, img_C, label_A, label_B, label_C, label12, label23 = rand_rot90_MCD(img_A, img_B, img_C, label_A, label_B, label_C, label12, label23)
    return rand_flip_MCD(img_A, img_B, img_C, label_A, label_B, label_C, label12, label23)
