...
from models.FEMCD.encoders.dual_vmamba import vssm_tiny as encoder
...
class MambaDecoder(nn.Module):
    ...
    def forward_up_features(self, inputs):  # inputs:[1, 96, 64, 64],[1, 192, 32, 32],[1, 384, 16, 16],[1, 768, 8, 8])
...
class FEMCD_net_dynamic(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(FEMCD_net_dynamic, self).__init__()
        ...
        if backbone == "resnet18" or backbone == "resnet34" or backbone == "resnet50" or backbone == "GrootV":
            self.Seg_Decoder = MambaDecoder(img_size=[512, 512], in_channels=self.channel_nums, num_classes=self.nclass, embed_dim=96, deep_supervision=False)
            ...
            self.Seg_Decoder = MambaDecoder(img_size=[512, 512], in_channels=self.channel_nums, num_classes=1, embed_dim=96, deep_supervision=False)
    ...
    def init_weights(self, pretrained=None):
        ...
        initialize_weights(self.Seg_Decoder, self.CD_Decoder)
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, pair):
        ...
        # SegDecoder
        out_1 = self.Seg_Decoder(out_A)
        out_2 = self.Seg_Decoder(out_B)
        out_3 = self.Seg_Decoder(out_C)
        out_4 = self.Seg_Decoder(out_D)
        out_5 = self.Seg_Decoder(out_E)
        out_6 = self.Seg_Decoder(out_F)
...
class FEMCD_net(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(FEMCD_net, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
    ...
    def forward(self, x1, x2, x3, pair):
        b, c, h, w = x1.shape
        out_A, out_B, out_C, outs_fused = self.backbone(x1, x2, x3, pair)
        # SegDecoder
...
class FEMCD_net_SECOND(nn.Module):
    def __init__(self, backbone, pretrained, nclass, lightweight, M, Lambda):
        super(FEMCD_net_SECOND, self).__init__()
        self.backbone_name = backbone
        self.nclass = nclass
        self.lightweight = lightweight
        self.M = M
