...
import math
import time
# from models.FEMCD.engine.logger import get_logger
from models.FEMCD.encoders.vmamba6 import Backbone_VSSM, CrossMambaFusionBlock, ConcatMambaFusionBlock
...
# designing for the WUSU dataset
class RGBXTransformer(nn.Module):
    def __init__(self,
                 num_classes=1000,
                 norm_layer=nn.LayerNorm,
    ...
    def forward_features(self, x_A, x_B, x_C, pair):
        """
        x_A: B x C x H x W
        """
        B = x_A.shape[0]
        ...
        for i in range(4):
            ...
            x_fuse = self.channel_attn_mamba[i](outs[pair[0]].permute(0, 2, 3, 1).contiguous(), outs[pair[-1]].permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()
            outs_fused.append(x_fuse)
        return outss_A, outss_B, outss_C, outs_fused

    def forward(self, x_A, x_B, x_C, pair):
        out = self.forward_features(x_A, x_B, x_C, pair)
        return out

# designing for the DYNAMIC dataset
# class RGBXTransformer(nn.Module):
#     def __init__(self,
#                  num_classes=1000,
#                  norm_layer=nn.LayerNorm,
...
#                                                 out[pair[-1]].permute(0, 2, 3, 1).contiguous()).permute(0, 3, 1, 2).contiguous()
#             outs_fused.append(x_fuse)
