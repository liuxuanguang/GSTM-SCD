...
from models.encoders.vmamba6 import CVSSDecoderBlock
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        B, H, W, C = x.shape
        x = rearrange(x, 'b h w (p1 p2 c)-> b (h p1) (w p2) c', p1=self.patch_size, p2=self.patch_size, c=C // (self.patch_size ** 2))
        x = self.norm(x)
        return x
...
class MambaDecoder(nn.Module):
    def __init__(self, 
        ...
        self.up = FinalUpsample_X4(input_resolution=(img_size[0] // patch_size, img_size[1] // patch_size), patch_size=4, dim=embed_dim)
