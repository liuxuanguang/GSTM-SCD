...
from models.Changeformer.ChangeFormerBaseNetworks import *
...
class ChangeFormerV6_encoder(nn.Module):

    def __init__(self, input_nc=3, output_nc=2, decoder_softmax=False, embed_dim=256):
        super(ChangeFormerV6_encoder, self).__init__()
        # Transformer Encoder
        self.embed_dims = [64, 128, 320, 512]
        self.depths = [3, 3, 4, 3]  # [3, 3, 6, 18, 3]
        self.embedding_dim = embed_dim
...
if __name__ == '__main__':
    ...
    # model = BiGrootV(backbone='resnet34', pretrained=True, nclass=7, lightweight=True, M=6, Lambda=0.00005).to(device)
    model = ChangeFormerV6().to(device)
    print(model)
    image1 = torch.randn(1, 1, 3, 512, 512).to(device)
    image2 = torch.randn(1, 3, 512, 512).to(device)
    # seg1, seg2, change = model(image1, image2)
    # print(seg1)
    from thop import profile
    FLOPs, Params = profile(model, (image1))
