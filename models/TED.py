...
class TED(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        return  F.upsample(out1, x_size[2:], mode='bilinear'), F.upsample(out2, x_size[2:], mode='bilinear'), F.upsample(change, x_size[2:], mode='bilinear').squeeze(1)


class TED_MT(nn.Module):
    def __init__(self, in_channels=4, num_classes=12):
        super(TED_MT, self).__init__()
        self.FCN = FCN(in_channels, pretrained=True)
        self.Dec1 = _DecoderBlock(128, 64, 128)
        self.Dec2 = _DecoderBlock(128, 64, 128)
    ...
    def forward(self, x1, x2, x3, pair):
        ...
        xs = [x1, x2, x3]
        xs_low = [x1_low, x2_low, x3_low]
        xc = self.CD_forward(xs[pair[0]], xs[pair[-1]])
        ...
        xc_low = torch.cat([xs_low[pair[0]], xs_low[pair[-1]]], 1)
        xc = self.DecCD(xc, xc_low)
        change = self.classifierCD(xc)
        ...
        return F.upsample(out1, x_size[2:], mode='bilinear'), F.upsample(out2, x_size[2:], mode='bilinear'), F.upsample(out3, x_size[2:], mode='bilinear'),F.upsample(
            change, x_size[2:], mode='bilinear').squeeze(1)

class TED_NMT(nn.Module):
    def __init__(self, in_channels=4, num_classes=7):
        super(TED_NMT, self).__init__()
        self.FCN = FCN(in_channels, pretrained=True)
        self.Dec1 = _DecoderBlock(128, 64, 128)
        self.Dec2 = _DecoderBlock(128, 64, 128)
        self.Dec3 = _DecoderBlock(128, 64, 128)
        ...
        initialize_weights(self.Dec1, self.Dec2, self.Dec3, self.Dec4, self.Dec5, self.Dec6,
                           self.classifier1, self.classifier2, self.classifier3, self.classifier4,  self.classifier5, self.classifier6, self.resCD, self.DecCD,
                           self.classifierCD)
    ...
    def forward(self, x1, x2, x3, x4, x5, x6):
        ...
        xc = self.CD_forward(x1, x6)

        x1 = self.Dec1(x1, x1_low)
        x2 = self.Dec2(x2, x2_low)
        x3 = self.Dec3(x3, x3_low)
        x4 = self.Dec4(x4, x4_low)
        x5 = self.Dec5(x5, x5_low)
        x6 = self.Dec6(x6, x6_low)
        out1 = self.classifier1(x1)
        out2 = self.classifier2(x2)
        out3 = self.classifier3(x3)
        ...
        xc_low = torch.cat([x1_low, x6_low], 1)
        xc = self.DecCD(xc, xc_low)
        change = self.classifierCD(xc)
...
# Debug  ：   there a refers to a sequence of images
a = torch.randn(1, 3, 512, 512).cuda()
b = torch.randn(1, 3, 512, 512).cuda()
model = TED(3, 7).cuda()
CD_out, SS_out1, SS_out2 = model(a, b)
xxx = 512