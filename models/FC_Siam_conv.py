...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.Dec3 = _DecoderBlock(128, 128 * 2, 64)
        self.Dec2 = _DecoderBlock(64, 64 * 2, 32)
        self.Dec1 = _DecoderBlock(32, 32 * 2, 16)
        self.Dec0 = _DecoderBlock(16, 16 * 2, 16)

    def forward(self, encs1, encs2):
        f1_enc0, f1_enc1, f1_enc2, f1_enc3, f1_enc4 = encs1
        f2_enc0, f2_enc1, f2_enc2, f2_enc3, f2_enc4 = encs2

        dec3 = self.Dec3(f1_enc4, torch.cat([f1_enc3, f2_enc3], 1))
        dec2 = self.Dec2(dec3, torch.cat([f1_enc2, f2_enc2], 1))
        dec1 = self.Dec1(dec2, torch.cat([f1_enc1, f2_enc1], 1))
        dec0 = self.Dec0(dec1, torch.cat([f1_enc0, f2_enc0], 1))
        return dec0
...
class FC_Siam_conv_Dynamic(nn.Module):
    def __init__(self, in_channels=4, num_classes=12):
        super(FC_Siam_conv_Dynamic, self).__init__()
        self.encoder = Encoder(in_channels)
        self.decoder = Decoder()
        self.classifier1 = nn.Conv2d(16, num_classes, kernel_size=1)
        self.classifier2 = nn.Conv2d(16, num_classes, kernel_size=1)
