...
from models.networks.multiltae import MultiLTAE
from models.networks.blocks import ConvBlock, DownConvBlock, UpConvBlock
...
if __name__ == '__main__':
    ...
    # 输入数据形状: [batch, time, channels, height, width]
    input_data = torch.randn(1, 2, 4, 512, 512).to(device)

    # 创建位置信息
    time_positions = torch.arange(1, 3).to(device)  # [6]
    batch_positions = time_positions[None, :, None, None]  # [1, 6, 1, 1]
    ...
    # 前向传播
    output = model(batch)
    print("输出形状:", output["logits"].shape)  # [1, 6, 8, 512, 512]
