...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        ...
        # LXG: 为了修改初始通道数增加的code
        updated_weights = state_dict.copy()  # 创建一个副本，避免修改原始权重

        for key, value in state_dict.items():
            if key == 'conv1.weight':
                # 检查当前模型的 conv1.weight 形状
                current_conv1_weight = model.conv1.weight  # 这里需要获取当前模型的 conv1.weight
                new_conv1_weight = torch.zeros_like(current_conv1_weight)
                # 将预训练权重的前3通道复制到新权重的前3通道
                new_conv1_weight[:, :3, :, :].copy_(value[:, :3, :, :])
                # 更新 updated_weights
