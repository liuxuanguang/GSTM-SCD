...
class _DecoderBlock(nn.Module):
    def __init__(self, in_ch, mid_channels, out_channels):
        super(_DecoderBlock, self).__init__()
        self.up = TransResBlock(in_ch, mid_channels)
        self.decode = nn.Sequential(ResBlock(mid_channels * 2, mid_channels),
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        self.Dec4 = _DecoderBlock(256, 128, 128)
        self.Dec3 = _DecoderBlock(128, 64, 64)
        self.Dec2 = _DecoderBlock(64, 32, 32)
        self.Dec1 = _DecoderBlock(32, 16, 16)
        self.Dec0 = _DecoderBlock(16, 8, 8)
...
class HRSCD_str3(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        super(HRSCD_str3, self).__init__()
        self.encoder = Encoder(in_channels)
        self.decoder = Decoder()
        self.encoder_cd = Encoder(in_channels * 2)
        self.decoder_cd = Decoder()
    ...
    def forward(self, x1, x2):
        ...
        out_cd = self.decoder_cd(encs_cd)
...
class HRSCD_str3_MT(nn.Module):
    def __init__(self, in_channels=4, num_classes=12):
        super(HRSCD_str3_MT, self).__init__()
        self.encoder = Encoder(in_channels)
        self.decoder = Decoder()
        self.encoder_cd = Encoder(in_channels * 2)
        self.decoder_cd = Decoder()
...
class HRSCD_str3_dymamic(nn.Module):
    def __init__(self, in_channels=4, num_classes=12):
        super(HRSCD_str3_dymamic, self).__init__()
        self.encoder = Encoder(in_channels)
        ...
        self.decoder_cd = Decoder()
        self.softmax = nn.Softmax(dim=1)
        self.classifier1 = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, num_classes, kernel_size=1))
        self.classifier2 = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, num_classes, kernel_size=1))
        self.classifier3 = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, num_classes, kernel_size=1))
        self.classifier4 = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, num_classes, kernel_size=1))
        self.classifier5 = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, num_classes, kernel_size=1))
        self.classifier6 = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, num_classes, kernel_size=1))
        self.classifier_CD = nn.Sequential(ResBlock(8, 8), nn.Conv2d(8, 1, kernel_size=1))
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, pair):
        ...
        out1 = self.decoder1(encs_s[0])
        out2 = self.decoder2(encs_s[1])
        out3 = self.decoder3(encs_s[2])
        out4 = self.decoder4(encs_s[3])
        out5 = self.decoder5(encs_s[4])
        out6 = self.decoder6(encs_s[5])
        out_cd = self.decoder_cd(encs_cd)

        out1 = self.classifier1(out1)
        out2 = self.classifier2(out2)
        out3 = self.classifier3(out3)
        out4 = self.classifier4(out4)
        out5 = self.classifier5(out5)
        out6 = self.classifier6(out6)
        out_cd = self.classifier_CD(out_cd)
