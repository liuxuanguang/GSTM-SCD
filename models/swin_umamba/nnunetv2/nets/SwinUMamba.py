...
class SwinUMamba_encoder(nn.Module):
    def __init__(
        self,
        in_chans=1,
        out_chans=13,
    ...
    def forward(self, x_in):
        ...
        fs.append(enc_hidden)

        return fs


class SwinUMamba_decoder(nn.Module):
    def __init__(
            self,
            out_chans=7,
            feat_size=[48, 96, 192, 384, 768],
    ...
    def forward(self, x_in):
        fs = x_in
        dec4 = self.decoder6(fs[-1], fs[4])
        dec3 = self.decoder5(dec4, fs[3])
        dec2 = self.decoder4(dec3, fs[2])
        dec1 = self.decoder3(dec2, fs[1])
        dec0 = self.decoder2(dec1, fs[0])
...
# model = SwinUMamba(in_chans=3, spatial_dims=2).to(device)
# # out = model(x)
# # print(out)
