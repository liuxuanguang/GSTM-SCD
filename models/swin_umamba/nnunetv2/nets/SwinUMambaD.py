...
from GrootV.classification.models.tree_scanning import Tree_SSM
import torch
...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2], 
        ...
        self.patch_embed = PatchEmbed2D(patch_size=4, in_chans=3, embed_dim=self.embed_dim,
...
class SwinUMambaD(nn.Module):
    def __init__(self,
        ...
        self.vssm_encoder = VSSMEncoder(self.inchas, self.patch_size, self.depths, self.dims, self.drop_path_rate)
        self.decoder = UNetResDecoder(self.num_classes, self.deep_supervision, self.features_per_stage, self.drop_path_rate, self.d_state,)
...
# x = torch.randn(1, 1, 3, 512, 512).to(device)
# model = SwinUMambaD().to(device)
# # out = model(x)
# # print(out)
