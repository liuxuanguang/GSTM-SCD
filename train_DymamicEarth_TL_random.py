...
from models.proposed_MTGrootV3D import MTGrootV_SV3_DynamicEarth_random as Net
...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--Net_name", type=str, default="MTGrootV_SV3_DynamicEarth_random_0610")
        parser.add_argument("--backbone", type=str, default="resnet34")
        parser.add_argument("--data_root", type=str, default=r"/media/lenovo/课题研究/博士小论文数据/语义变化检测数据集/DynamicEarthNet_4band/train/DynamicEarth512")
...
class ChangeConsistencyLoss(nn.Module):
    ...
    def cosine_distance(self, p, q):
        # p, q: (B, C, H, W)
        sim = F.cosine_similarity(p, q, dim=1)  # (B, H, W)
        return (1 - sim) / 2  # Range [0,1]

    def forward(self, preds_a, preds_b, preds_c):
        # preds: (B, C, H, W)
        T_ab = self.cosine_distance(preds_a, preds_b)
        T_bc = self.cosine_distance(preds_b, preds_c)
        T_ac = self.cosine_distance(preds_a, preds_c)

        # 规则违反项计算
        term1 = T_ab * T_bc * (1 - T_ac)
        term2 = (1 - T_ab) * (1 - T_bc) * T_ac
        term3 = T_ab * (1 - T_bc) * (1 - T_ac)
...
class Trainer:
    def __init__(self, args):
        ...
        self.TCL = ChangeConsistencyLoss()
    ...
    def training(self, epoch):
        ...
        for i, (img1, img2, img3, img4, img5, img6, mask1, mask2, mask3, mask4, mask5, mask6, mask_bn, id) in enumerate(tbar):
            ...
            out, out_bn, pair = self.model(img1, img2, img3, img4, img5, img6)    #[b,6,512,512],[b,6,512,512],[b,512,512]
            ...
            loss_TL = self.TCL(out[0], out[3], out[-1])
            loss = (loss_bn + loss_seg + loss_similarity)*0.9 + loss_TL*0.1
    ...
    def validation(self, epoch):
        ...
        with torch.no_grad():
            for img1, img2, img3, img4, img5, img6, mask1, mask2, mask3, mask4, mask5, mask6, mask_bn, id in tbar:
                ...
                out, out_bn, pair = self.model(img1, img2, img3, img4, img5, img6)
                mask = [mask1, mask2, mask3, mask4, mask5, mask6]
                mask_bn = mask[pair[0]] - mask[pair[1]]
                mask_bn[mask_bn != 0] = 1
