...
class Options:
    def __init__(self):
        ...
        parser.add_argument("--Net_name", type=str, default="FC_Siam_conv-全部时相")
        ...
        parser.add_argument("--load_from", type=str,
                            default=r"/media/lenovo/课题研究/博士小论文数据/长时序变化检测/Long-term-SCD/CMSCD_lxg/checkpoints/DynamicEarth/FCSiamconv_epoch84_Score19.61_mIOU53.33_Sek5.15_Fscd37.90_OA82.73.pth")
...
def inference_scd(args, pair):
    ...
    with torch.no_grad():
        for img1, img2, img3, img4, img5, img6, label1, label2, label3, label4, label5, label6, label_bn, id in tbar:
            ...
            out1, out2 = model(img1, img2, img3, img4, img5, img6, pair)

            # 直接获取预测结果，避免不必要的张量复制
            seg_predictions = [torch.argmax(out, dim=1).cpu().numpy() for out in [out1, out2]]
            change_prediction = seg_predictions[0]
            change_prediction = (change_prediction > 0).astype(np.uint8) * 255
            # 只处理相关时相的预测结果
            time1_pred = seg_predictions[0]
            time2_pred = seg_predictions[1]

            # 转换标签为numpy数组
            labels_np = {
                'time1': label1.numpy().astype(np.uint8) if pair[0] == 0 else
                         label2.numpy().astype(np.uint8) if pair[0] == 1 else
                         label3.numpy().astype(np.uint8) if pair[0] == 2 else
                         label4.numpy().astype(np.uint8) if pair[0] == 3 else
                         label5.numpy().astype(np.uint8) if pair[0] == 4 else
                         label6.numpy().astype(np.uint8),
                'time2': label1.numpy().astype(np.uint8) if pair[1] == 0 else
                         label2.numpy().astype(np.uint8) if pair[1] == 1 else
                         label3.numpy().astype(np.uint8) if pair[1] == 2 else
                         label4.numpy().astype(np.uint8) if pair[1] == 3 else
                         label5.numpy().astype(np.uint8) if pair[1] == 4 else
                         label6.numpy().astype(np.uint8),
                'change': label_bn.numpy().astype(np.uint8)
            }
            ...
            # 添加到指标计算
            metric.add_batch(time1_pred, time1_gt_masked)
            metric.add_batch(time2_pred, time2_gt_masked)

            # 保存预测结果
            for idx in range(time1_pred.shape[0]):
                # 保存语义变化图1
                mask1 = Image.fromarray(time1_pred[idx].astype(np.uint8)).convert('P')
                mask1.putpalette(cmap)
                mask1.save(os.path.join(pred_save_path1, id[idx]))

                # 保存语义变化图2
                mask2 = Image.fromarray(time2_pred[idx].astype(np.uint8)).convert('P')
                mask2.putpalette(cmap)
                mask2.save(os.path.join(pred_save_path2, id[idx]))

                # 保存二值变化图
                change_img = Image.fromarray(change_prediction[idx]).convert('L')
