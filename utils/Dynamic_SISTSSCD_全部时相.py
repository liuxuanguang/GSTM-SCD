...
def inference_scd(args):
    ...
    pred_save_path4 = os.path.join(pair_dir, 'pred_semantic_time4')
    pred_save_path5 = os.path.join(pair_dir, 'pred_semantic_time5')
    pred_save_path6 = os.path.join(pair_dir, 'pred_semantic_time6')
    ...
    os.makedirs(pred_save_path4, exist_ok=True)
    os.makedirs(pred_save_path5, exist_ok=True)
    os.makedirs(pred_save_path6, exist_ok=True)
    ...
    # MultiUTAE
    # model = Net(input_dim=4, num_classes=len(RS.ST_CLASSES), in_features=512, T=6)
    # UNet3D
    model = Net(in_channel=4, n_classes=len(RS.ST_CLASSES), timesteps=6, dropout=0.2)
    if args.load_from:
    ...
    with torch.no_grad():
        for img1, img2, img3, img4, img5, img6, label1, label2, label3, label4, label5, label6, label_bn, id in tbar:
            ...
            # 用于MultiUTAE
            # # 创建位置信息并确保为float32
            # time_positions = torch.arange(1, 7).float().cuda()  # 添加.float()
            ...
            # LCs = [out1.squeeze(0), out2.squeeze(0), out3.squeeze(0), out4.squeeze(0), out5.squeeze(0), out6.squeeze(0)]
            # metric.add_batch(out1, label1.numpy())
            # metric.add_batch(out2, label2.numpy())
            # metric.add_batch(out3, label3.numpy())
            # metric.add_batch(out4, label4.numpy())
            # metric.add_batch(out5, label5.numpy())
            ...
            for idx in range(batch_size):
                # 保存语义变化图1
                mask1 = Image.fromarray(LCs[0].astype(np.uint8)).convert('P')
                mask1.putpalette(cmap)
                mask1.save(os.path.join(pred_save_path1, id[idx]))

                mask2 = Image.fromarray(LCs[1].astype(np.uint8)).convert('P')
                mask2.putpalette(cmap)
                mask2.save(os.path.join(pred_save_path2, id[idx]))

                mask3 = Image.fromarray(LCs[2].astype(np.uint8)).convert('P')
                mask3.putpalette(cmap)
                mask3.save(os.path.join(pred_save_path3, id[idx]))

                mask4 = Image.fromarray(LCs[3].astype(np.uint8)).convert('P')
                mask4.putpalette(cmap)
                mask4.save(os.path.join(pred_save_path4, id[idx]))

                mask5 = Image.fromarray(LCs[4].astype(np.uint8)).convert('P')
                mask5.putpalette(cmap)
                mask5.save(os.path.join(pred_save_path5, id[idx]))

                mask6 = Image.fromarray(LCs[5].astype(np.uint8)).convert('P')
                mask6.putpalette(cmap)
                mask6.save(os.path.join(pred_save_path6, id[idx]))
