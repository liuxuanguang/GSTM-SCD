...
class CrossScan(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        ...
        xs[:, 1] = x.reshape([B,C,H,W//2,2]).transpose(2,3).reshape([B,C,H,W]).flatten(2, 3) 
    ...
    def backward(ctx, ys: torch.Tensor):
        ...
        y = ys[:, 0] + ys[:, 1].reshape(B, -1, W//2, H, 2).transpose(2,3).contiguous().view(B, -1, L)    
...
class CrossMerge(torch.autograd.Function):
    ...
    def forward(ctx, ys: torch.Tensor):
        ...
        y = ys[:, 0] + ys[:, 1].reshape(B, -1, W//2, H, 2).transpose(2,3).contiguous().view(B, D, -1)   
        return y
...
class SS2D(nn.Module):
    ...
    def forwardv2(self, x: torch.Tensor, **kwargs):
        ...
        x = self.in_proj(x)  #(b, h, w, d)-->(1, 64, 128, 512)
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1)) # (b, h, w, d)-->(1,64,128,256)
